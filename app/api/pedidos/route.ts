...
export async function POST(req:NextRequest){try{const body=await req.json();const whatsapp=telefoneValido(body.whatsapp);const nomeCliente=String(body.nome_cliente||"").trim().slice(0,100);const total=Number(body.total);const itens=Array.isArray(body.itens)?body.itens as ItemEntrada[]:[];
 ...
 const{data,error}=await database.from("pedidos").insert({pedido_id:pedidoId,whatsapp,nome_cliente:nomeCliente,total:Number(total.toFixed(2)),status:"pendente",selo_creditado:false,itens:itensValidos}).select("pedido_id,whatsapp,nome_cliente,total,status,criado_em,selo_creditado").single();
