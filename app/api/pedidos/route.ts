...
export async function POST(req:NextRequest){try{const body=await req.json();const whatsapp=telefoneValido(body.whatsapp);const nomeCliente=String(body.nome_cliente||"").trim().slice(0,100);const total=Number(body.total);const itens=Array.isArray(body.itens)?body.itens as ItemEntrada[]:[];
 ...
 const pedidoId=`PED-${Date.now()}-${crypto.randomBytes(3).toString("hex")}`;const database=db();
 const{data,error}=await database.from("pedidos").insert({pedido_id:pedidoId,whatsapp,nome_cliente:nomeCliente,total:Number(total.toFixed(2)),status:"pendente",selo_creditado:false,itens:itensValidos}).select("pedido_id,whatsapp,nome_cliente,total,status,criado_em,selo_credititado,selo_creditado").single();
 if(error){if(String(error.message||"").toLowerCase().includes("itens")){const fallback=await database.from("pedidos").insert({pedido_id:pedidoId,whatsapp,nome_cliente:nomeCliente,total:Number(total.toFixed(2)),status:"pendente",selo_creditado:false}).select("pedido_id,whatsapp,nome_cliente,total,status,criado_em,selo_creditado").single();if(fallback.error)throw fallback.error;return NextResponse.json({pedido:fallback.data,criado:true,aviso:"Banco ainda precisa da coluna itens"},{status:201})}throw error}
 return NextResponse.json({pedido:data,criado:true},{status:201});
