...
async function pedidosPorStatus(database:ReturnType<typeof db>,status:string,limite=50){const{data,error}=await database.from("pedidos").select("id,pedido_id,whatsapp,nome_cliente,total,status,selo_creditado,criado_em,confirmado_em").eq("status",status).order(status==="confirmado"?"confirmado_em":"criado_em",{ascending:false}).limit(limite);if(error)throw error;return data||[]}
async function pedidosPendentes(database:ReturnType<typeof db>){return pedidosPorStatus(database,"pendente",50)}
...
export async function POST(req:NextRequest){
 try{
  ...
  if(body.action==="listar_confirmados")return NextResponse.json({pedidos:await pedidosPorStatus(database,"confirmado",50)});
  if(body.action==="confirmar_pedido"){
   ...
   const totalAnterior=Number(cliente.total_gasto||0);const{error:seloErro}=await database.rpc("creditar_selo_fidelidade",{p_telefone:tel,p_pedido_id:pedido.pedido_id});if(seloErro)throw seloErro;
   const{error:gastoErro}=await database.from("clientes_fidelidade").update({total_gasto:Number((totalAnterior+Number(pedido.total||0)).toFixed(2)),atualizado_em:new Date().toISOString()}).eq("id",cliente.id);if(gastoErro)throw gastoErro;
   const agora=new Date().toISOString();const{data:marcado,error:mErro}=await database.from("pedidos").update({status:"confirmado",selo_creditado:true,confirmado_em:agora}).eq("id",pedido.id).eq("status","pendente").eq("selo_creditado",false).select("id").maybeSingle();if(mErro)throw mErro;if(!marcado)return NextResponse.json({error:"O pedido já foi processado"},{status:409});
  ...
  const tel=telefone(body.telefone);if(!tel)return NextResponse.json({error:"WhatsApp inválido"},{status:400});const{data:cliente,error:buscaErro}=await database.from("clientes_fidelidade").select("id,nome,telefone,pontos,total_pedidos,total_gasto,atualizado_em").eq("telefone",tel).maybeSingle();if(buscaErro)throw buscaErro;if(!cliente)return NextResponse.json({error:"Cliente não encontrado"},{status:404});
  ...
  else return NextResponse.json({error:"Ação inválida"},{status:400});return NextResponse.json(await dadosCliente(database,cliente.id));
