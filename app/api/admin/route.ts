...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.action === "login") {
      ...
      const { data, error } = await database.rpc("verificar_admin_popular", { p_usuario: usuario, p_senha: senha });
      if (error) throw error;
      if (!data) return NextResponse.json({ error: "Usuário ou senha incorretos" }, { status: 401 });
      return NextResponse.json({ token: makeToken() });
    }

    if (!validToken(req.headers.get("authorization")?.replace(/^Bearer\s+/i, "") || null))
      return NextResponse.json({ error: "Sessão inválida ou expirada" }, { status: 401 });

    const tel = telefone(body.telefone);
    if (!tel) return NextResponse.json({ error: "WhatsApp inválido" }, { status: 400 });

    const { data: cliente, error: buscaErro } = await database.from("clientes_fidelidade")
      .select("id,nome,telefone,pontos,total_pedidos,total_gasto,atualizado_em")
      .eq("telefone", tel).maybeSingle();
    if (buscaErro) throw buscaErro;
    if (!cliente) return NextResponse.json({ error: "Cliente não encontrado" }, { status: 404 });

    if (body.action === "buscar") return NextResponse.json({ cliente });

    if (body.action === "adicionar") {
      const pedidoId = `ADMIN-${Date.now()}-${crypto.randomBytes(3).toString("hex")}`;
      const { error } = await database.rpc("creditar_selo_fidelidade", { p_telefone: tel, p_pedido_id: pedidoId });
      if (error) throw error;
    ...
    const { data: atualizado, error } = await database.from("clientes_fidelidade")
      .select("id,nome,telefone,pontos,total_pedidos,total_gasto,atualizado_em")
