...
export default function Fidelidade() {
  ...
  return <main className="min-h-screen bg-zinc-950 text-white p-4 pb-28 max-w-2xl mx-auto">
    ...
    <section className={`mt-4 ${completo ? "bg-zinc-900" : "bg-yellow-400 text-black"} rounded-3xl p-6`}>
      <div className="flex justify-between"><div><p className="font-bold opacity-60">SEU CARTÃO</p><h2 className="text-4xl font-black">{Math.min(selos, META)}/{META}</h2></div>{!completo && <div className="text-right"><p className="text-xs font-bold opacity-60">FALTAM</p><b className="text-2xl">{faltam}</b></div>}</div>
      <div className="grid grid-cols-4 gap-3 mt-5">{Array.from({ length: META }).map((_, i) => <div key={i} className={`aspect-square rounded-full flex items-center justify-center text-xl border-2 ${i < selos ? (completo ? "bg-yellow-400 text-black" : "bg-black text-yellow-400") : "bg-white/20 border-current/20"}`}>{i < selos ? "★" : i + 1}</div>)}</div>
      <div className="h-3 bg-black/20 rounded-full mt-5 overflow-hidden"><div className={`h-full rounded-full ${completo ? "bg-green-400" : "bg-black"}`} style={{ width: `${progresso}%` }} /></div>
    </section>
