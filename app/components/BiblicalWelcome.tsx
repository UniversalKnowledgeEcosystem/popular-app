...
export default function BiblicalWelcome(){const[aberto,setAberto]=useState(false);const[m,setM]=useState(mensagens[0]);useEffect(()=>{const anterior=Number(sessionStorage.getItem("popular_versiculo_indice")||"-1");let i=Math.floor(Math.random()*mensagens.length);if(mensagens.length>1&&i===anterior)i=(i+1)%mensagens.length;sessionStorage.setItem("popular_versiculo_indice",String(i));setM(mensagens[i]);setAberto(true);const t=window.setTimeout(()=>setAberto(false),2800);return()=>window.clearTimeout(t)},[]);if(!aberto)return null;return <div className="fixed inset-0 z-[9999] bg-zinc-950 flex items-center justify-center p-7 text-white" role="dialog" aria-label="Mensagem bíblica de boas-vindas"><div className="max-w-md text-center animate-[fadeIn_.35s_ease-out]"><div className="relative mx-auto w-28 h-32 overflow-hidden rounded-3xl bg-zinc-900 border border-yellow-400/30 shadow-2xl"><Image src="/MASCOTE%20POPULAR.jpeg" alt="Mascote da Popular" fill priority sizes="112px" className="object-contain object-bottom"/></div><p className="mt-5 text-yellow-400 text-xs font-black tracking-[.2em]">POPULAR • SINCE 2017</p><h1 className="text-2xl font-black mt-4 leading-tight">“{m.texto}”</h1><p className="text-yellow-400 font-bold mt-3">📖 {m.ref}</p><p className="text-zinc-500 text-xs mt-6">Que Deus abençoe o seu dia. ❤️</p><button onClick={()=>setAberto(false)} className="mt-5 text-xs text-zinc-400 underline underline-offset-4">Entrar agora</button></div></div>}
