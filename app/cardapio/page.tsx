...
const categorias = ["Todos", "Favoritos", "Lanches", "Batatas", "Bebidas", "Sucos", "Açaí", "Sorvetes", "Milk Shakes"];

function volumeDoProduto(nome: string) {
  const ml = nome.match(/(\d+(?:[.,]\d+)?)\s*ml\b/i);
  if (ml) return Number(ml[1].replace(",", "."));
  const litros = nome.match(/(\d+(?:[.,]\d+)?)\s*(?:litro|litros|l)\b/i);
  if (litros) return Number(litros[1].replace(",", ".")) * 1000;
...
function IconeProduto({ item }: { item: Item }) {
  ...
  const escala = Math.max(0.78, Math.min(1.42, 0.72 + volume / 1800));
  const largura = Math.round(31 * escala);
  const altura = Math.round(42 * escala);
...
function C() {
  ...
  const filtrados = itens.filter(i => {
    ...
    return (categoria === "Todos" || (categoria === "Favoritos" ? fav.includes(i.id) : i.categoria === categoria)) && (!q || i.nome.toLowerCase().includes(q));
  });
  ...
  return <main className="min-h-screen bg-black text-white pb-40">
    <div className="sticky top-0 z-40 bg-black/95 border-b border-zinc-800 p-4">
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between"><div><p className="text-yellow-400 text-xs font-black">CARDÁPIO ATUALIZADO</p><h1 className="text-2xl font-black">Popular</h1></div><Link href="/">Início</Link></div>
        <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="🔎 O que você quer comer?" className="w-full bg-zinc-900 p-3 rounded-xl mt-3" />
        <div className="flex gap-2 overflow-x-auto pt-3">{categorias.map(c => <button key={c} onClick={() => setCategoria(c)} className={`whitespace-nowrap px-4 py-2 rounded-full text-sm font-bold ${categoria === c ? "bg-yellow-400 text-black" : "bg-zinc-900"}`}>{c}</button>)}</div>
      </div>
    </div>
    <div className="max-w-5xl mx-auto p-4 grid sm:grid-cols-2 gap-3">
      {filtrados.map(i => <article key={i.id} className={`relative rounded-2xl p-4 flex gap-3 items-center border ${i.disponivel ? "bg-zinc-900 border-zinc-800" : "bg-zinc-950 border-zinc-900 opacity-60"}`}>
        {i.destaque && <span className="absolute -top-2 left-3 bg-yellow-400 text-black text-[10px] font-black px-2 py-1 rounded-full">🔥 DESTAQUE</span>}
        <IconeProduto item={i} />
        <div className="flex-1 min-w-0"><div className="flex justify-between gap-2"><b>{i.nome}</b><button onClick={() => favorito(i.id)}>{fav.includes(i.id) ? "❤️" : "🤍"}</button></div>{!i.disponivel ? <p className="text-red-400 font-black text-sm mt-2">● ESGOTADO NO MOMENTO</p> : <>{i.normal && <span className="text-xs text-zinc-500 line-through mr-2">{formatPrice(i.normal)}</span>}<b className={i.normal ? "text-green-400" : "text-yellow-400"}>{formatPrice(i.preco)}</b>{i.normal && <span className="text-[10px] bg-green-500 text-black font-black ml-2 px-2 py-1 rounded-full">OFERTA</span>}</>}</div>
        <button disabled={!i.disponivel} onClick={() => abrir(i)} className={`w-11 h-11 shrink-0 rounded-xl text-2xl font-black ${i.disponivel ? "bg-yellow-400 text-black" : "bg-zinc-800 text-zinc-600"}`}>{i.disponivel ? "+" : "×"}</button>
      </article>)}
    </div>
    {toast && <div className="fixed top-4 z-[90] left-4 right-4 max-w-sm mx-auto bg-green-500 text-black p-3 rounded-xl text-center font-black">{toast}</div>}
    ...
    {quantidadeTotal > 0 && <Link href="/carrinho" className="fixed bottom-20 left-4 right-4 max-w-md mx-auto bg-yellow-400 text-black rounded-2xl p-4 flex justify-between font-black"><span>🛒 {quantidadeTotal} itens</span><span>{formatPrice(total)} →</span></Link>}
  </main>;
}
